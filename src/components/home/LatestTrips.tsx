...
import { Plane, Calendar, Users, Star, MapPin, ArrowRight, Loader2, AlertCircle } from 'lucide-react';
...
export default function LatestTrips() {
  ...
  const { data, isLoading, error } = useQuery<{ data: Trip[] }>({
    ...
    queryFn: async () => {
      const response = await fetch(`${serverUrl}/api/trips/public?limit=3`);
      ...
      <section className="relative overflow-hidden bg-slate-950 px-6 py-20 text-white">
        <div className="container mx-auto max-w-7xl">
          <div className="p-6 rounded-2xl bg-red-500/10 border border-red-500/20 backdrop-blur-xl flex items-center gap-4">
            ...
            <span className="text-red-400">Failed to load latest trips</span>
          </div>
        </div>
      </section>
    );
        ...
        >
          ...
          <div className="mb-12">
            ...
            <p className="text-slate-400 max-w-2xl">
              Discover what other travelers are planning. Get inspired by their itineraries and start
              your own journey.
            </p>
          </div>
          ...
          >
            {trips.map((trip) => (
              ...
              >
                ...
                {/* Content */}
                <div className="p-6">
                  <h3 className="text-xl font-semibold text-white mb-2 line-clamp-1">{trip.title}</h3>
                  ...
                  {/* View Button */}
                  <Button
                    className="w-full border border-white/20 text-white hover:bg-white/5 transition-all"
                  >
                    View Itinerary
